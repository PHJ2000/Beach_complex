...
export function BeachDetailView({
  ...
  const initialPositionX = -((selectedMarkerPosition.x - 50) * 5.76);
  const initialPositionY = -((selectedMarkerPosition.y - 35) * 7.2);
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0 flex-shrink">
            ...
            <div className="min-w-0">
              <h1 className="font-['Noto_Sans_KR:Bold',_sans-serif] text-[15px] leading-tight text-foreground whitespace-nowrap">
                비치체크
              </h1>
            ...
            <>
              <TransformComponent wrapperClass="w-full h-full" contentClass="w-full h-full">
                <div className="relative w-full h-full min-w-[480px] min-h-[600px]">
                  ...
                  {allBeaches.map((item) => {
                    const isSelected = item.id === beach.id;
                    const position = computeMarkerPosition(item) ?? (isSelected ? selectedMarkerPosition : null);
                          ...
                          <div className="relative">
                            ...
                            <div className="absolute top-[70px] left-1/2 -translate-x-1/2 whitespace-nowrap">
                              <div className="bg-white px-4 py-2 rounded-full shadow-lg border-2 border-[#007DFC]">
                                <p className="font-['Noto_Sans_KR:Bold',_sans-serif] text-[#007DFC]">{item.name}</p>
                              </div>
                            </div>
                          </div>
                        ) : (
                          ...
                          >
                            <div className="relative w-[35px] h-[42px]">
                              <svg className="absolute inset-0 w-full h-full drop-shadow-md" fill="none" preserveAspectRatio="none" viewBox="0 0 29 34">
                                <path
                                  clipRule="evenodd"
                                  d={svgPaths.p1a91f00}
                                  fill={STATUS_COLORS[item.status] ?? STATUS_COLORS.unknown}
                                  fillRule="evenodd"
                                  stroke="#ffffff"
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                            ...
                            <div className="absolute top-[50px] left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
                              <div className="bg-gray-900 text-white px-3 py-1.5 rounded-lg shadow-lg">
                                <p className="font-['Noto_Sans_KR:Bold',_sans-serif] text-[11px]">{item.name}</p>
                  ...
                  <div className="relative">
                    <div className="flex items-end justify-between gap-[2px] h-[180px]">
                      {hourlyConditions.map((data) => {
                        ...
                        return (
                          <div key={data.hour} className="flex flex-col items-center gap-2 flex-1 relative group">
                            <div className="absolute bottom-full mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                              <div className="bg-gray-900 text-white px-3 py-2 rounded-lg shadow-lg whitespace-nowrap">
                                <p className="font-['Noto_Sans_KR:Bold',_sans-serif] text-[12px]">{data.hour}시</p>
